...
function C_FORMS_SOURCE(props) {

	var p 				= props.props;
	var now 			= new Date().getTime();
	var state_			= p.form_data || WH.defaults.source_form; // {state: {}, fields: {}}
	...
	const l 				= {general: WH.locales.GET(lang, 'content.form.general'), item: WH.locales.GET(lang, 'content.form.source')};

	var disabled_submit 	= form.state.phase === 'sending' ? true : false;
	var source_name 		= (p.source) ? p.source.name : ''; // for edit purposes
	var headers 			= {add: l.item.new_header, edit: l.item.edit_header+' '+source_name};
	...
	function submit(e) {
		...
		if(form.state.valid && form.state.phase !== 'sending' && rights.write ) {
			...
			var edit_c_id	= p.mode === 'edit' ? p.source.id : 0;
			...
			if( !dummy ) {
				...
				.then((result) => {
					...
					if(result.ok) {
						...
						if(p.mode === 'add') 	S.items.handle_upsert_add({result, items: sources, set_items: set_sources, section, content, set_mode: p.set_mode, l, alert, set_alert});
						if(p.mode === 'edit') 	S.items.handle_upsert_edit({result, items: sources, set_items: set_sources, section, content, set_mode: p.set_mode, l, alert, set_alert});

					} else { set_form({type: 'set_errors', result}); M.log.error({id: '[ACTILOG_SFORM_01]', message:  'SOCKET RESULT ERROR', result}); }

				}).catch((error) => {

					set_form({type: 'set_phase', phase: 'input'});
					set_form({type: 'set_errors', result: {id: '[ACTSF02]', text: 'Unknown error - '+error.message, error}});
					M.log.error({id: '[ACTILOG_SFORM_02]', message:'SOCKET ERROR', error});
			...
			// if dummy, do not send to the server
				...
				try { S.items.dummy_upsert({p, content, items: sources, l, alert, set_alert, set_items: set_sources, form, lang, to_send, set_mode: p.set_mode, section}); } 
				catch(e) { set_form({type: 'set_errors', result: {id: '[ACTSF02.1]', text: 'Unknown error - '+e.message, error: e}}); M.log.error({id: '[ACTILOG_SFORM_02]', message:'DUMMY SOCKET ERROR', error: e}); }
	...
	return ( 
		...
		<form className="content_form source_form">

			<div className="form_row form_row_source"><div className="fr_label fr_label_source"></div><div className="fr_header fr_header_source">{header}</div></div>

			<C.FORMS_INPUTS_TEXT 		input={field_data.name} 				data={{l, section, lang, form, set_form}} />
	
			<div className="form_row form_row_source">

				<div className="fr_label fr_label_source"></div>
				<div className="fr_input fr_input_source" style={{marginTop: '10px'}}>
